...
/// <summary>
/// Service for monitoring system resources (memory, CPU, disk space).
/// Provides periodic resource usage tracking and alerts for high usage.
/// </summary>
public sealed class ResourceMonitorService : IDisposable
{
    private readonly System.Timers.Timer _resourceCheckTimer;
    ...
    {
        // PERFORMANCE: Increase interval to reduce overhead
        _resourceCheckTimer = new System.Timers.Timer(TimeSpan.FromMinutes(5).TotalMilliseconds);
        _resourceCheckTimer.Elapsed += MonitorResources;
        _resourceCheckTimer.AutoReset = true;
    }
    ...
    {
        _resourceCheckTimer.Start();
        Log.Information("📊 Resource monitoring started");
    }
    ...
    {
        _resourceCheckTimer.Stop();
        Log.Information("📊 Resource monitoring stopped");
    }
    ...
    /// <summary>
    /// Disposes of the resource monitor.
    /// </summary>
    public void Dispose()
    {
        Dispose(true);
