...
/// Includes comprehensive logging for operations and user interactions.
/// </summary>
public partial class MainViewModel : ObservableObject, IDisposable
{
    ...
    public MainViewModel(
        ...
        GrokSupercomputer grokSupercomputer,
    ...
    public Models.BudgetMetrics BudgetMetrics => _enterpriseViewModel?.BudgetMetrics ?? new Models.BudgetMetrics();

    /// <summary>Selected theme for the application.</summary>
    [ObservableProperty]
    private string selectedTheme = "FluentLight";

    /// <summary>Loading progress for async operations.</summary>
    [ObservableProperty]
    private double loadingProgress;

    /// <summary>Loading message for user feedback.</summary>
    [ObservableProperty]
    private string loadingMessage = "Ready";

    /// <summary>Indicates if any operation is busy.</summary>
    [ObservableProperty]
    private bool isBusy;
    ...
    private void HandleEnterpriseSelectionChanged()
    {
        if (_enterpriseViewModel != null)
        {
            _enterpriseViewModel.HandleSelectionChanged();
        }
    }

    /// <summary>Command to handle SfDataGrid RowDoubleTapped event.</summary>
    [RelayCommand]
    private void HandleEnterpriseRowDoubleTapped()
    {
        if (_enterpriseViewModel != null)
        {
            _enterpriseViewModel.HandleRowDoubleTapped();
        }
    }

    /// <summary>Command to handle theme changes from SfSkinManager.</summary>
    [RelayCommand]
    private void HandleThemeChanged(string themeName)
    {
        if (!string.IsNullOrEmpty(themeName))
        {
            ...
            Log.Information("MainViewModel theme changed to: {Theme}", themeName);
        }
    }

    /// <summary>Cycles to the next widget (wrap-around). If none selected selects first; safe for empty list.</summary>
    [RelayCommand]
    ...
    /// <summary>Adds a sample widget with incremental Id for UI testing (non-persistent).</summary>
    [RelayCommand]
    ...
    private async System.Threading.Tasks.Task LoadEnterprisesAsync()
    {
        try
        {
            IsBusy = true;
            LoadingProgress = 0;
            LoadingMessage = "Starting enterprise load...";
            ...
            // Update progress through the enterprise view model
            _enterpriseViewModel.PropertyChanged += (s, e) =>
            {
                if (e.PropertyName == nameof(_enterpriseViewModel.LoadingProgress))
                {
                    LoadingProgress = _enterpriseViewModel.LoadingProgress;
                }
                if (e.PropertyName == nameof(_enterpriseViewModel.LoadingMessage))
                {
                    LoadingMessage = _enterpriseViewModel.LoadingMessage;
                }
            };
            ...
            LoadingMessage = "Load complete";
        }
        catch (Exception ex)
        {
            Log.Error(ex, "Error in LoadEnterprisesAsync");
...
Output structured analysis with actionable insights.";
    ...
    protected virtual void Dispose(bool disposing)
    {
        ...
        if (disposing)
        {
            ...
            _dbContext?.Dispose();
            (_enterpriseViewModel as IDisposable)?.Dispose();
        }
        _disposed = true;
    }
