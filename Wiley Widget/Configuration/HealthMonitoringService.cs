...
public class HealthMonitoringService
{
    ...
    /// <summary>
    /// Converts Microsoft.Extensions.Diagnostics.HealthChecks.HealthStatus to WileyWidget.HealthStatus
    /// </summary>
    private static HealthStatus ConvertHealthStatus(Microsoft.Extensions.Diagnostics.HealthChecks.HealthStatus status)
    {
    ...
    public async Task<HealthReport> CheckHealthAsync()
    {
        try
        {
            ...
            {
                var status = ConvertHealthStatus(entry.Value.Status);
    ...
    public async Task<HealthStatusSummary> GetHealthSummaryAsync()
    {
        ...
        {
            OverallStatus = ConvertHealthStatus(report.Status),
            TotalDuration = report.TotalDuration,
            CheckCount = report.Entries.Count,
            HealthyCount = report.Entries.Count(e => ConvertHealthStatus(e.Value.Status) == HealthStatus.Healthy),
            DegradedCount = report.Entries.Count(e => ConvertHealthStatus(e.Value.Status) == HealthStatus.Degraded),
            UnhealthyCount = report.Entries.Count(e => ConvertHealthStatus(e.Value.Status) == HealthStatus.Unhealthy),
            CheckResults = report.Entries.ToDictionary(
                e => e.Key,
                e => new HealthCheckResult
                {
                    Status = ConvertHealthStatus(e.Value.Status),
    ...
    {
        var report = await CheckHealthAsync();
        return ConvertHealthStatus(report.Status) == HealthStatus.Healthy;
    }
}
