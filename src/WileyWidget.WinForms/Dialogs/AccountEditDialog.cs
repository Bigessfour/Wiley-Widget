...
{
    /// <summary>
    /// Modal dialog for creating and editing MunicipalAccount entities.
    ...
    public sealed class AccountEditDialog : Form
    {
        ...
        private readonly ILogger<AccountEditDialog> _logger;
        ...
        private ComboBox? _fundComboBox;
        private ComboBox? _departmentComboBox;
        private ComboBox? _budgetPeriodComboBox;
        ...
        public AccountEditDialog(
            ...
            ILogger<AccountEditDialog> logger,
        ...
        private void InitializeDialog()
        {
            ...
            _accountNumberTextBox = new TextBox
            {
                Dock = DockStyle.Fill,
                Font = new Font("Segoe UI", 9F),
                Margin = new Padding(0, 5, 0, 10),
                PlaceholderText = "e.g., 405, 405.1, 101-1000"
            };
            ...
            _nameTextBox = new TextBox
            {
                ...
                Font = new Font("Segoe UI", 9F),
                Margin = new Padding(0, 5, 0, 10),
                PlaceholderText = "Descriptive account name",
                MaxLength = 100
            };
            ...
            _typeComboBox.Items.AddRange(Enum.GetNames(typeof(AccountType)).Cast<object>().ToArray());
            mainPanel.Controls.Add(_typeComboBox, 1, row++);

            // Fund Type
            mainPanel.Controls.Add(CreateLabel("Fund Type:*"), 0, row);
            _fundComboBox = new ComboBox
            {
                Dock = DockStyle.Fill,
                DropDownStyle = ComboBoxStyle.DropDownList,
                Font = new Font("Segoe UI", 9F),
            ...
            _fundComboBox.Items.AddRange(Enum.GetNames(typeof(MunicipalFundType)).Cast<object>().ToArray());
            ...
            _budgetPeriodComboBox = new ComboBox
            {
                Dock = DockStyle.Fill,
                DropDownStyle = ComboBoxStyle.DropDownList,
                Font = new Font("Segoe UI", 9F),
            ...
            _parentAccountComboBox.Items.Add("(None)");
            ...
            _budgetAmountNumeric = new NumericUpDown
            {
                Dock = DockStyle.Fill,
                Font = new Font("Segoe UI", 9F),
                Margin = new Padding(0, 5, 0, 10),
        ...
        private async Task LoadReferenceDataAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                // Add parent accounts to combo (with "(None)" already added in InitializeDialog)
                foreach (var parent in _potentialParents)
                {
                    _parentAccountComboBox!.Items.Add(new ComboBoxItem
                    {
                        Text = $"{parent.AccountNumber?.Value} - {parent.Name}",
                        Value = parent.Id
                    });
                }
        ...
        private void PopulateExistingData()
        {
            ...
            _balanceNumeric!.Value = _existingAccount.Balance;
            _budgetAmountNumeric!.Value = _existingAccount.BudgetAmount;
            ...
            {
                var parentItem = _parentAccountComboBox!.Items
                    .Cast<object>()
                    .FirstOrDefault(item =>
                        item is ComboBoxItem cbi && (int)cbi.Value == _existingAccount.ParentAccountId.Value);
                if (parentItem != null)
        ...
        private async Task SaveButton_Click()
        {
            try
            {
                ...
                account.Type = Enum.Parse<AccountType>(_typeComboBox!.SelectedItem?.ToString() ?? "Asset");
                account.Fund = Enum.Parse<MunicipalFundType>(_fundComboBox!.SelectedItem?.ToString() ?? "General");
                account.Balance = _balanceNumeric!.Value;
                account.BudgetAmount = _budgetAmountNumeric!.Value;
