...
namespace WileyWidget.WinForms.Controls.Panels
{
    ...
    public partial class DashboardPanel : ScopedPanelBase<FormsMainViewModel>
    {
        ...
        /// <returns>LegacyGradientPanel containing gauge with label</returns>
        private LegacyGradientPanel CreateGaugePanel(string title, string iconName)
        {
            var panelTheme = SfSkinManager.ApplicationVisualTheme ?? WileyWidget.WinForms.Themes.ThemeColors.DefaultTheme;
            var container = new LegacyGradientPanel
            {
                Width = 280,
                Height = 120,
            ...
            // Label panel
            var lblPanel = new LegacyGradientPanel
            {
                Dock = DockStyle.Fill,
                BorderStyle = BorderStyle.None,
                BackgroundColor = new BrushInfo(GradientStyle.Vertical, Color.Empty, Color.Empty)
                ...
                if (e.PropertyName == nameof(_vm.TotalBudgetGauge))
                {
                    try
                    {
                        if (_budgetUtilizationGauge != null && !_budgetUtilizationGauge.IsDisposed)
                        {
                            ...
                            var valueLabel = _budgetUtilizationGauge.Parent?.Controls.OfType<LegacyGradientPanel>().FirstOrDefault()?.Controls.OfType<Label>().FirstOrDefault(l => l.Name.Contains("Value", StringComparison.Ordinal));
                ...
                if (e.PropertyName == nameof(_vm.RevenueGauge))
                {
                    try
                    {
                        if (_revenueGauge != null && !_revenueGauge.IsDisposed)
                        {
                            ...
                            var valueLabel = _revenueGauge.Parent?.Controls.OfType<LegacyGradientPanel>().FirstOrDefault()?.Controls.OfType<Label>().FirstOrDefault(l => l.Name.Contains("Value", StringComparison.Ordinal));
                ...
                if (e.PropertyName == nameof(_vm.ExpensesGauge))
                {
                    try
                    {
                        if (_expenseGauge != null && !_expenseGauge.IsDisposed)
                        {
                            ...
                            var valueLabel = _expenseGauge.Parent?.Controls.OfType<LegacyGradientPanel>().FirstOrDefault()?.Controls.OfType<Label>().FirstOrDefault(l => l.Name.Contains("Value", StringComparison.Ordinal));
                ...
                if (e.PropertyName == nameof(_vm.NetPositionGauge))
                {
                    try
                    {
                        if (_varianceGauge != null && !_varianceGauge.IsDisposed)
                        {
                            ...
                            var valueLabel = _varianceGauge.Parent?.Controls.OfType<LegacyGradientPanel>().FirstOrDefault()?.Controls.OfType<Label>().FirstOrDefault(l => l.Name.Contains("Value", StringComparison.Ordinal));
