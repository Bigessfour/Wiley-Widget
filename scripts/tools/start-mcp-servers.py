...
def start_servers(dry_run: bool = False) -> None:
    ...
    venv_python = venv_path / "Scripts" / "python.exe" if Path.cwd().drive else venv_path / "bin" / "python"
    
    # Start syncfusion-docs (assume MCP command; adjust if custom)
    sync_cmd = [str(venv_python), "-m", "mcp_server_fetch", "--server", "syncfusion-docs"]  # Placeholder; use actual entrypoint
    run_cmd(sync_cmd, dry_run=dry_run)
    
    # Start syncfusion-docs-node (Node.js? Assume npm if mixed)
    node_cmd = ["npm", "start", "--", "syncfusion-docs-node"]  # If Node-based; check .continue/mcpServers/
