...
possible_build = REPO_ROOT / "src" / "WileyWidget.WinForms" / "bin" / "Debug" / "net9.0-windows"
...
def run_static_view_checks(repo_root: Path):
    ...
    vm_files = []
    if forms_dir:
        form_files = [f for f in sorted(forms_dir.rglob("*Form.cs")) if "Designer.cs" not in f.name]
    if vms_dir:
    ...
    def check_form(fpath: Path):
        ...
        if not ("BindingSource" in text or "DataBindings.Add" in text or "DataSource =" in text):
            res['issues'].append('No data binding / BindingSource / DataSource usage')

        # 5. Accessibility markers
        if not ("AccessibleName" in text or "AccessibleDescription" in text or "AccessibleRole" in text):
            res['issues'].append('Missing AccessibleName/AccessibleDescription')

        # 6. ThemeManager or SfSkinManager
        if not ("ThemeManager" in text or "SfSkinManager" in text or "Syncfusion" in text):
            # not mandatory for tiny helper dialogs, but warn
            res['issues'].append('ThemeManager/SfSkinManager/Syncfusion theming not detected')
        ...
        if len(bad_literals) > 5:
            res['issues'].append(f'Too many hard-coded strings in InitializeComponent ({len(bad_literals)})')

        # 8. No direct DB/service calls in view
        if re.search(r"\b(DbContext|UnitOfWork|WileyWidget\.Business|WileyWidget\.Data)\b", text):
        ...
        btn_defs = re.findall(r"(\w+)\s*=\s*new\s+(?:\w+\.|)?(?:Button|SfButton|ToolStripButton)\b", text)
        for btn in set(btn_defs):
            ...
            if not re.search(pattern, text):
                res['issues'].append(f'Button "{btn}" missing themed icon assignment (iconService.GetIcon)')
    ...
    def check_vm(fpath: Path):
        ...
        if not ("INotifyPropertyChanged" in text or "[ObservableProperty]" in text or "ObservableObject" in text or "ObservableRecipient" in text):
            res['issues'].append('ViewModel does not implement INotifyPropertyChanged / ObservableProperty')

        # 2. Contains at least one RelayCommand / AsyncRelayCommand / ICommand
        if not re.search(r"RelayCommand|AsyncRelayCommand|ICommand|\[RelayCommand\]", text):
            res['issues'].append('No commands found in ViewModel (RelayCommand/AsyncRelayCommand/ICommand)')
        ...
        if not ("FluentValidation" in text or "ObservableValidator" in text or "Validation" in text or "IValidatableObject" in text):
...
def main_entry():
    ...
    try:
        ...
        forms = static_details.get('forms', []) if isinstance(static_details, dict) else []
        if forms:
            print("\nForms:")
            for f in forms:
                issues = f.get('issues', [])
        ...
        vms = static_details.get('viewmodels', []) if isinstance(static_details, dict) else []
        if vms:
            print("\nViewModels:")
            for v in vms:
                issues = v.get('issues', [])
