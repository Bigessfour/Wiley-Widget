...
class RepositoryAnalyzer:
    ...
    def _categorize_file(self, file_path: Path) -> tuple[str, str]:
        ...
        if extension in [
            ...
            return "configuration", (
                "build"
                if "build" in path_str or extension in [".props", ".targets"]
                else "settings"
            )

        # Documentation
        if (
            extension in [".md", ".txt", ".rst"]
            or "docs/" in path_str
            or "documentation/" in path_str
        ):
            sub_cat = (
                "api"
                if "api" in path_str
                    ...
                    "guide"
                    if any(x in path_str for x in ["guide", "tutorial"])
                    else "general"
                )
            )
        ...
        if extension in [".cs", ".csproj", ".xaml"]:
            ...
            if extension == ".cs":
                ...
                if "service" in path_str or filename.endswith("service.cs"):
                    if (
                        "abstractions" in path_str
                        or filename.startswith("i")
                        and filename.endswith("service.cs")
                    ):
        ...
        if (
            ...
            return (
                ...
                (
                    "unit_test"
                    if "Unit" in path_str
                    else "integration_test" if "Integration" in path_str else ""
                ),
            )
        ...
        elif path_str.startswith("scripts/") or file_path.suffix in [
            ".ps1",
            ".psm1",
            ".psd1",
        ]:
            ...
            return (
                ...
                (
                    "build"
                    if file_path.suffix in [".csproj", ".sln", ".props", ".targets"]
                    else ""
                ),
            )
        elif path_str.startswith("docker/") or file_path.name.lower().startswith(
            "dockerfile"
        ):
    ...
    def _build_dependency_graph(
        ...
        graph["top_dependencies"] = [
            {"name": dep, "usage_count": count} for dep, count in sorted_deps[:20]
        ]
    ...
    def _generate_architecture_summary(
        ...
        # Track view-viewmodel relationships
        view_vm_map: dict[str, dict[str, Any]] = {}
        ...
        for file_data in files_data:
            ...
            name = Path(path).stem
            summary_text = file_data["context"].get("summary", "")
            ...
            if category == "source_code":
                if sub_category == "ui":
                    ...
                    elif "QuickBooks" in name or "Qbo" in name:
                        service_info["service_type"] = "External Integration"
                    elif "Budget" in name or "Account" in name:
                        service_info["service_type"] = "Business Logic"
                    elif "Export" in name or "Import" in name:
                    ...
                    if "DbContext" in name or "Context" in name:
                        ...
                        # Extract DbSet information from summary
                        if "DbSet" in summary_text:
                            dbset_matches = re.findall(r"DbSet<(\w+)>", summary_text)
                            if dbset_matches:
                                data_info["entities"] = dbset_matches[:20]
                        summary["dbcontexts"].append(data_info)
                    else:
    ...
    def _calculate_metrics(
        ...
        for file_data in files_data:
            ...
            if "src/" in path:
                ...
                project_metrics[project]["lines_of_code"] += metadata.get(
                    "line_count", 0
                )
    ...
    def _build_folder_tree(self, files: list[Path]) -> dict[str, str | list]:
        ...
        def sort_tree(node):
            if "children" in node:
                ...
                truncated = [
                    c for c in node["children"] if c.get("type") == "truncated"
                ]
                non_truncated = [
                    c for c in node["children"] if c.get("type") != "truncated"
                ]
        ...
        print(
            f"  Folder tree: {tree_stats['dirs']} directories, {tree_stats['files']} files, depth={tree_stats['max_depth']}"
        )
    ...
    def generate_manifest(
        ...
        if max_files_in_manifest and len(files_data) > max_files_in_manifest:
            ...
            files_data_sorted = sorted(
                ...
                key=lambda x: (
                    (
                        0
                        if x["context"]["importance"] == "high"
                        else 1 if x["context"]["importance"] == "normal" else 2
                    ),
        ...
        print(
            f"   Files: {len(files_data)}"
            + (f" of {original_file_count} (truncated)" if files_truncated else "")
        )
        ...
        print(
            f"   Architecture: {architecture['counts']['viewmodels']} ViewModels, {architecture['counts']['views']} Views"
        )
        print(
...
def main():
    ...
    if args.categories:
        ...
        if invalid_cats:
            print(
                f"Warning: Unknown categories will be ignored: {', '.join(invalid_cats)}"
            )
