...
EXCLUDE_DIRS = {'.git', 'node_modules', 'bin', 'obj', '.venv', '__pycache__', 'TestResults', 'logs', 'xaml-logs', '.vs'}
TEXT_EXTS = {'.md', '.txt', '.json', '.yaml', '.yml', '.py', '.ps1', '.cs', '.csproj', '.sln', '.xml', '.xaml', '.sql', '.sh', '.psd1', '.psm1', '.gitignore', '.gitattributes', '.editorconfig', '.toml', '.html', '.htm', '.csv'}
LANG_BY_EXT = {
...
def make_fetch_url(owner_repo: str | None, commit: str | None, branch: str | None, relpath: str) -> str | None:
    if not owner_repo:
        return None
...
def scan_files(root: Path, include_snippets=True, compute_full_hash=True, max_files=10000) -> tuple[list[dict], dict]:
    files = []
        ...
        try:
            if p.is_dir():
                ...
                for hint, cname in CATEGORY_HINTS:
                    if any(hint in part for part in lower_parts) or hint.lower() in str(rel).lower() or ext.lower().strip('.') == hint:
                        cat = cname
                        break
                if cat == 'unknown':
                    if ext.lower() in {'.md', '.markdown'}:
                        ...
                        cat = 'configuration'
                    elif ext.lower() in {'.cs', '.py', '.sh', '.ps1', '.java', '.ts', '.js'}:
                        cat = 'source_code'
...
def build_manifest(root: Path, out: Path, include_snippets=True, compute_full_hash=True, max_files=10000) -> dict:
    now = datetime.utcnow().isoformat()
    git = repo_git_info(root)
    files, totals = scan_files(root, include_snippets=include_snippets, compute_full_hash=compute_full_hash, max_files=max_files)

    # enhance files with fetch_url when possible
    for f in files:
        f['fetch_url'] = make_fetch_url(git.get('owner_repo'), git.get('commit_hash'), git.get('branch'), f['path'])
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Generate rich ai-fetchable-manifest.json for repo')
    parser.add_argument('-o', '--output', default='ai-fetchable-manifest.json', help='Output file path (relative to repo root)')
    parser.add_argument('-r', '--root', default='.', help='Repository root path (default: .)')
    parser.add_argument('--no-snippets', dest='snippets', action='store_false', help='Do not include text snippets in manifest')
    parser.add_argument('--no-full-hash', dest='fullhash', action='store_false', help='Do not compute full SHA1 hashes')
    parser.add_argument('--max-files', dest='max_files', type=int, default=10000, help='Maximum files to include in manifest')
    ...
    manifest = build_manifest(repo_root, out_path, include_snippets=args.snippets, compute_full_hash=args.fullhash, max_files=args.max_files)
