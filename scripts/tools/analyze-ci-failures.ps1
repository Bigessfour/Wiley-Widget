...
.DESCRIPTION
    Analyzes CI pipeline failures and applies automated fixes where possible.
    Integrates with GitHub Actions, Trunk CLI, and local development tools.
.PARAMETER WorkflowRunId
    GitHub workflow run ID to analyze
.PARAMETER AutoFix
    Automatically apply fixes when possible
.PARAMETER Verbose
    Enable verbose output
.EXAMPLE
    .\analyze-ci-failures.ps1 -WorkflowRunId 123456789 -AutoFix
#>
...
3. Fix compilation errors before merging
4. Ensure all tests pass in CI environment
5. Run `trunk check --ci` locally before pushing

---
*Generated by CI Failure Analysis Script*
"@
