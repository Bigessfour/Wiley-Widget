...
async function searchSyncfusion(query) {
  return new Promise((resolve, reject) => {
    ...
    const apiKey = process.env.BRIGHTDATA_API_KEY || process.env.API_TOKEN;
    if (!apiKey) {
      return reject(
        new Error("Missing BRIGHTDATA_API_KEY environment variable"),
      );
    }
    const options = {
...
async function main() {
  ...
  console.log(
    "🔍 Searching Syncfusion documentation for resource conflicts...\n",
  );
