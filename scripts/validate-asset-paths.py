...
class AssetValidator:
    ...
    def validate_index_html(self) -> bool:
        ...
        for check, description in checks.items():
            if check in content:
                ...
                self.validations.append({"check": description, "status": "PASS"})
            else:
    ...
    def validate_dotnet_packages(self) -> bool:
        ...
        for package, version_hint in required_packages.items():
            if package in content:
                ...
                self.validations.append({"check": package, "status": "PASS"})
            else:
                self.warnings.append(
                    {
                        "severity": "WARNING",
        ...
        for component in component_files:
            if component.exists():
                ...
                self.validations.append(
                    {"check": f"Component {component.name}", "status": "PASS"}
                )
            else:
                self.warnings.append(
                    {
    ...
    def print_report(self):
        ...
        print("\n" + "=" * 70)
        print(
            f"Status: {summary['status']} | Issues: {summary['total_issues']} | Warnings: {summary['total_warnings']}"
        )
        print("=" * 70)
