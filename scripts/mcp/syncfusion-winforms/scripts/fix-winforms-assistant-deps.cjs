...
const targetPkgPath = path.join(root, 'node_modules', '@syncfusion', 'winforms-assistant', 'package.json');
...
try {
  ...
  if (!pkg.dependencies[desired]) {
    ...
    try {
      ...
      execSync(`npm install ${desired}@${desiredRange} --no-save`, { cwd: installDir, stdio: 'inherit' });
