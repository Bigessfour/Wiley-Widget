...
public static class MockFactory
{
        ...
        Exception? exception = null)
    {
        ...
        if (shouldThrow)
        {
            var ex = exception ?? new InvalidOperationException("Service error");
            mockService.Setup(s => s.GetDashboardItemsAsync(It.IsAny<CancellationToken>()))
                .ThrowsAsync(ex);
        }
        else
        {
...
public static class TestExtensions
{
    ...
    public static void VerifyLog<T>(this Mock<ILogger<T>> logger, LogLevel level, string message)
    {
        ...
        logger.Verify(x => x.Log(
            level,
            It.IsAny<EventId>(),
            It.Is<It.IsAnyType>((o, t) => o != null && o.ToString().Contains(message)),
            It.IsAny<Exception>(),
