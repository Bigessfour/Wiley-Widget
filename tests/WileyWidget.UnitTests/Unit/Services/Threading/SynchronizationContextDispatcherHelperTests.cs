...
namespace WileyWidget.WinForms.Tests.Unit.Services.Threading
{
    public class SynchronizationContextDispatcherHelperTests : IDisposable
    {
        ...
        [Fact]
        public async Task Invoke_FromBackgroundThread_ExecutesOnUiThread()
        {
            SynchronizationContextDispatcherHelper? helper = null;
            int uiThreadId = 0;
            bool ranOnUi = false;
        ...
        [Fact]
        public async Task InvokeT_FromBackgroundThread_ReturnsValueFromUiThread()
        {
            SynchronizationContextDispatcherHelper? helper = null;
            int uiThreadId = 0;
            ...
            var result = await Task.Run(() => helper!.Invoke(() => Thread.CurrentThread.ManagedThreadId));
        ...
        public async Task InvokeAsync_PostsToUiThread()
        {
            ...
            await helper!.InvokeAsync(() => ranOnUi = Thread.CurrentThread.ManagedThreadId == uiThreadId);
