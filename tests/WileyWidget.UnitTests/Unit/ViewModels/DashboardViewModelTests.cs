...
public sealed class DashboardViewModelTests : IDisposable
{
    ...
    public async Task LoadDashboardDataAsync_ComputesFY2026_FromDecember2025Date()
    {
        ...
        using var vm = new DashboardViewModel(NullLogger<DashboardViewModel>.Instance, budgetRepo, accountRepo, config);
    ...
    public async Task Dispose_CancelsPendingLoad_AndReleasesResources()
    {
        ...
        var vm = new DashboardViewModel(NullLogger<DashboardViewModel>.Instance, budgetRepo, accountRepo, config);
    ...
    public async Task LoadDashboard_StopsRetries_OnObjectDisposedException()
    {
        ...
        using var vm = new DashboardViewModel(NullLogger<DashboardViewModel>.Instance, budgetRepo, accountRepo);
