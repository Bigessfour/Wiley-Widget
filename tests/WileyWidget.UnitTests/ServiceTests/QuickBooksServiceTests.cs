...
namespace WileyWidget.Services.Tests.ServiceTests
{
    ...
    public class QuickBooksServiceTests
    {
        ...
        [Fact]
        public async Task SyncDataAsync_CreatesTransaction_WhenCustomFieldMappingExists()
        {
            // Arrange
            var mockSettings = new Mock<ISettingsService>();
            var appSettings = new AppSettings();
            ...
            var invoice = new Intuit.Ipp.Data.Invoice
            {
                ...
                CustomField = new[] { new Intuit.Ipp.Data.CustomField { Name = "BudgetCategoryId", AnyIntuitObject = "42" } }
            };

            var mockDs = new Mock<IQuickBooksDataService>();
            mockDs.Setup(x => x.FindInvoices(It.IsAny<int>(), It.IsAny<int>())).Returns(new List<Intuit.Ipp.Data.Invoice> { invoice });

            var mockTransactionRepo = new Mock<ITransactionRepository>();
            mockTransactionRepo.Setup(x => x.GetAllAsync()).ReturnsAsync(new List<WileyWidget.Models.Transaction>());

            WileyWidget.Models.Transaction? addedTransaction = null;
            mockTransactionRepo.Setup(x => x.AddAsync(It.IsAny<WileyWidget.Models.Transaction>()))
                .Callback<WileyWidget.Models.Transaction>(t => addedTransaction = t)
                .Returns(Task.CompletedTask);

            var mockMappingService = new Mock<IQuickBooksMappingService>();
            // The mapping service now exposes a resolver that accepts an Invoice object
            mockMappingService.Setup(x => x.ResolveBudgetEntryIdForInvoiceAsync(It.IsAny<Intuit.Ipp.Data.Invoice>())).ReturnsAsync(55);

            var mockServiceProvider = new Mock<IServiceProvider>();
            mockServiceProvider.Setup(sp => sp.GetService(typeof(IQuickBooksMappingService))).Returns(mockMappingService.Object);
            ...
            Assert.Equal(55, addedTransaction!.BudgetEntryId);
        }

        [Fact]
        public async Task SyncDataAsync_ExistingTransactionAmountMismatch_LogsAndUpdatesTransaction()
        {
            // Arrange
            var mockSettings = new Mock<ISettingsService>();
            var appSettings = new AppSettings();
            ...
            var existingTransaction = new WileyWidget.Models.Transaction
            {
                Id = 1,
                QuickBooksInvoiceId = "INV-200",
                Amount = 100.00m,
                TransactionDate = DateTime.UtcNow.AddDays(-1)
            };

            WileyWidget.Models.Transaction? updatedTransaction = null;
            var mockTransactionRepo = new Mock<ITransactionRepository>();
            mockTransactionRepo.Setup(x => x.GetAllAsync()).ReturnsAsync(new List<WileyWidget.Models.Transaction> { existingTransaction });
            mockTransactionRepo.Setup(x => x.UpdateAsync(It.IsAny<WileyWidget.Models.Transaction>()))
                .Callback<WileyWidget.Models.Transaction>(t => updatedTransaction = t)
                .Returns(Task.CompletedTask);

            var mockActivityRepo = new Mock<WileyWidget.Business.Interfaces.IActivityLogRepository>();
            mockActivityRepo.Setup(x => x.LogActivityAsync(It.IsAny<WileyWidget.Models.ActivityLog>(), It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);

            var mockServiceProvider = new Mock<IServiceProvider>();
            mockServiceProvider.Setup(sp => sp.GetService(typeof(WileyWidget.Business.Interfaces.IActivityLogRepository))).Returns(mockActivityRepo.Object);
            ...
            Assert.Equal(110.00m, updatedTransaction!.Amount);
            mockActivityRepo.Verify(x => x.LogActivityAsync(It.Is<WileyWidget.Models.ActivityLog>(a => a.ActivityType == "QuickBooks.SyncConflict" && a.Details.Contains("OldAmount")), It.IsAny<CancellationToken>()), Times.Once);
        }

        [Fact]
        public async Task SyncDataAsync_KeepLocal_PreservesLocalAndLogsConflict()
        {
            // Arrange
            var mockSettings = new Mock<ISettingsService>();
            var appSettings = new AppSettings { QuickBooksConflictPolicy = QuickBooksConflictPolicy.KeepLocal };
            mockSettings.SetupGet(x => x.Current).Returns(appSettings);
            mockSettings.Setup(x => x.LoadAsync()).Returns(Task.CompletedTask);
            ...
            var mockTransactionRepo = new Mock<ITransactionRepository>();
            mockTransactionRepo.Setup(x => x.GetAllAsync()).ReturnsAsync(new List<WileyWidget.Models.Transaction> { existingTransaction });
            mockTransactionRepo.Setup(x => x.UpdateAsync(It.IsAny<WileyWidget.Models.Transaction>())).Returns(Task.CompletedTask);
            ...
            // Assert
            Assert.True(result.Success);
            // Update should not be called when policy is KeepLocal
            mockTransactionRepo.Verify(x => x.UpdateAsync(It.IsAny<WileyWidget.Models.Transaction>()), Times.Never);
            mockActivityRepo.Verify(x => x.LogActivityAsync(It.Is<WileyWidget.Models.ActivityLog>(a => a.ActivityType == "QuickBooks.SyncConflict" && a.Details.Contains("Policy=KeepLocal")), It.IsAny<CancellationToken>()), Times.Once);
        }

        [Fact]
        public async Task SyncDataAsync_PromptUser_FlagsForManualResolution()
        {
            // Arrange
            var mockSettings = new Mock<ISettingsService>();
            var appSettings = new AppSettings { QuickBooksConflictPolicy = QuickBooksConflictPolicy.PromptUser };
            mockSettings.SetupGet(x => x.Current).Returns(appSettings);
            mockSettings.Setup(x => x.LoadAsync()).Returns(Task.CompletedTask);
            ...
            Assert.True(result.Success);
            mockTransactionRepo.Verify(x => x.UpdateAsync(It.IsAny<WileyWidget.Models.Transaction>()), Times.Never);
            mockActivityRepo.Verify(x => x.LogActivityAsync(It.Is<WileyWidget.Models.ActivityLog>(a => a.ActivityType == "QuickBooks.SyncConflict" && a.Details.Contains("Policy=PromptUser")), It.IsAny<CancellationToken>()), Times.Once);
        }
