...
- The Testcontainers-based fixture reads the Postgres password from the `POSTGRES_TEST_PASSWORD` environment variable. If that variable is not set, a cryptographically-secure, runtime-only password will be generated by the fixture. For local runs, the helper script `./scripts/docker/start-postgres.ps1` will set a default password of `WileyTestPassw0rd!` for convenience (you may override it with `-PostgresTestPassword` or export `POSTGRES_TEST_PASSWORD` prior to running the script).
- For CI runs, set a repository secret named `POSTGRES_TEST_PASSWORD` and reference it in your workflow for reproducible runs and easier debugging. The integration test workflow forwards this secret to the job environment when present.
