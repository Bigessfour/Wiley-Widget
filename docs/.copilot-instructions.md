...
## Project Overview
Wiley Widget is a .NET 8.0 WPF application with comprehensive CI/CD pipeline using GitHub Actions, Trunk for code quality, and Azure deployment capabilities. The application features AI-powered budget analytics through xAI's Grok API integration, enabling intelligent municipal enterprise management with "What If" scenario planning and automated optimization recommendations.

### 🚨 CRITICAL CI/CD REQUIREMENT
**EXCLUSIVE CI/CD METHOD**: This project mandates the use of the validated `ci-optimized.yml` pipeline with Trunk CLI v1.25.0 integration and merge queue support. All other CI/CD approaches are prohibited and will be rejected.

**Required Components**:
- ✅ Trunk CLI integration for security scanning and code quality
- ✅ Merge queue support with automated quality gates
...
- **AI/ML**: xAI Grok API integration via GrokSupercomputer service

## AI/ML Features & GrokSupercomputer Integration

### GrokSupercomputer Overview
GrokSupercomputer is Wiley Widget's AI-powered calculation engine that leverages xAI's Grok API for advanced budget analytics, reducing local complexity by 60% while adding enterprise-grade intelligence.
...
- **Real-time Insights**: Dynamic analysis with caching for performance

#### Integration Points:
- **Service Layer**: `Services/GrokSupercomputer.cs` - Core AI processing
- **ViewModels**: `MainViewModel.AnalyzeBudgetWithGrokAsync()` - UI integration
- **Models**: Enhanced with AI-computed properties and insights
- **Caching**: 15-30 minute result caching for cost optimization
...
var prompt = $@"You are a municipal budget expert. Analyze this data: {dataJson}
Provide specific recommendations in this exact JSON format: {jsonSchema}";
```
...
catch (Exception ex)
{
    ...
    return ComputeLocally(enterprises); // Graceful degradation
}
```

#### 3. **Cost Optimization**
- **Request Batching**: Combine multiple calculations in single API call
- **Smart Caching**: Cache results for 15-30 minutes
- **Usage Monitoring**: Track API costs with budget alerts
- **Progressive Enhancement**: Basic features work without AI

#### 4. **Security & Privacy**
- **Data Anonymization**: Remove sensitive identifiers from AI prompts
...
#### When to Use GrokSupercomputer:
- ✅ **Complex Calculations**: Multi-variable budget analysis
- ✅ **Predictive Modeling**: Revenue forecasting and risk assessment
- ✅ **Intelligent Insights**: Automated recommendations and optimization
- ✅ **Scenario Analysis**: "What If" modeling with confidence intervals

#### When to Use Local Calculations:
- ❌ **Simple Arithmetic**: Basic addition/subtraction
- ❌ **Real-time UI Updates**: Instant feedback requirements
- ❌ **Offline Functionality**: Must work without internet
- ❌ **Sensitive Data**: PII or confidential information

#### Implementation Pattern:
```csharp
// AI-enhanced method with fallback
public async Task<decimal> CalculateComplexMetric()
{
    if (_grokSupercomputer != null && await IsOnlineAsync())
    {
        try
        {
            return await _grokSupercomputer.ComputeAdvancedMetricAsync(this);
        }
        catch
        {
...
```csharp
[Fact]
public async Task GrokSupercomputer_HandlesApiErrors_Gracefully()
{
    // Arrange: Mock API failure
    var mockClient = new Mock<HttpClient>();
    mockClient.Setup(c => c.PostAsJsonAsync(It.IsAny<string>(), It.IsAny<object>()))
        .ThrowsAsync(new HttpRequestException());
    
    // Act & Assert: Should fallback to local calculation
    var result = await _grokSupercomputer.ComputeWithFallback(enterprises);
    Assert.NotNull(result);
}
```

#### Integration Testing:
- **API Mocking**: Test with fake Grok responses
- **Offline Mode**: Validate fallback functionality
- **Performance**: Ensure <5s response times
- **Cost Monitoring**: Track API usage in test environments
...
# Cost Management
XAI_DAILY_BUDGET=10.00
XAI_MONTHLY_BUDGET=300.00
```

### AI Feature Development Workflow

#### 1. **Planning Phase**
- Define AI use case and success criteria
- Design prompt structure and response format
- Plan fallback mechanisms and error handling

#### 2. **Implementation Phase**
- Implement GrokSupercomputer method
- Add caching and performance optimizations
- Integrate with existing ViewModels and UI
...
```csharp
/// <summary>
/// Calculates complex budget metrics using AI for enhanced accuracy
/// Falls back to local calculation if AI service is unavailable
/// </summary>
/// <param name="enterprises">List of enterprises to analyze</param>
/// <returns>AI-enhanced budget metrics</returns>
public async Task<BudgetMetrics> ComputeBudgetAnalyticsAsync(List<Enterprise> enterprises)
```

#### User Documentation:
- Clear indication of AI vs. local processing
- Explanation of AI capabilities and limitations
- Cost transparency and usage monitoring
- Offline mode functionality description
...
- **Optimization Algorithms**: Advanced rate recommendations
- **Real-time Analysis**: Streaming data processing

#### Implementation Goals:
- **Scalability**: Handle large datasets efficiently
- **Intelligence**: Custom model training for local patterns
- **Integration**: Multi-modal input support
- **Collaboration**: Multi-user scenario sharing

---

## CI/CD Workflow Methods & Best Practices

### 🚨 EXCLUSIVE CI/CD METHOD REQUIREMENT

**CRITICAL**: This project uses ONLY the validated CI/CD pipeline with Trunk CLI integration and merge queue support. All other CI/CD methods are deprecated and must not be used.

**Exclusive Method**: `ci-optimized.yml` with Trunk v1.25.0 integration
- ✅ **REQUIRED**: Trunk security scanning, code quality analysis, and analytics
- ✅ **REQUIRED**: Merge queue support with automated quality gates
- ✅ **REQUIRED**: Parallel test execution with coverage reporting
- ✅ **REQUIRED**: Comprehensive monitoring and health validation

**Prohibited Methods**:
- ❌ Manual CI/CD workflows without Trunk integration
- ❌ Legacy pipelines without merge queue support
- ❌ Custom build scripts bypassing the validated pipeline
- ❌ Direct deployments without CI/CD validation

### Primary CI/CD Workflows

#### 1. EXCLUSIVE: CI/CD 90% Success Rate Pipeline (`ci-optimized.yml`)
**Status**: ✅ ACTIVE AND REQUIRED - This is the ONLY CI/CD method to use
**Purpose**: Enterprise-grade CI/CD with Trunk integration and merge queue support
**Triggers**: Push to main/temp, PR to main/temp, merge_group, schedule, workflow_dispatch
**Key Features**:
- ✅ Trunk CLI v1.25.0 with security scanning and analytics
- ✅ Merge queue support with automated quality gates
- ✅ Parallel test execution (4 parallel jobs)
- ✅ Comprehensive security scanning (Gitleaks, Trufflehog, OSV-Scanner)
- ✅ Code quality analysis with PSScriptAnalyzer and Prettier
- ✅ Health validation and circuit breaker logic
- ✅ Automated artifact generation and deployment

```yaml
# EXCLUSIVE CI/CD Pipeline - REQUIRED for all deployments
name: CI/CD 90% Success Rate (Trunk Integrated)
on:
  push:
    branches: [main, temp]
  pull_request:
...
#### 2. DEPRECATED: Legacy CI/CD Pipeline (`comprehensive-cicd.yml`)
**Status**: ❌ DEPRECATED - DO NOT USE
**Reason**: Lacks Trunk integration and merge queue support
**Replacement**: Use `ci-optimized.yml` exclusively

#### 3. DEPRECATED: Merge Queue Pipeline (`merge-queue-cicd.yml`)
**Status**: ❌ DEPRECATED - DO NOT USE
**Reason**: Functionality integrated into `ci-optimized.yml`
**Replacement**: Use `ci-optimized.yml` exclusively
...
```powershell
# EXCLUSIVE: Validate current CI/CD setup
.\scripts\trunk-cicd-monitor.ps1 -Validate -HealthCheck

# EXCLUSIVE: Monitor merge queue status
.\scripts\merge-queue-manager.ps1 -Status -Verbose

# EXCLUSIVE: Run Trunk analysis locally
trunk check --all --upload

# EXCLUSIVE: Generate CI/CD status report
.\scripts\trunk-cicd-monitor.ps1 -Report -OutputJson
...
**BEFORE ANY DEPLOYMENT OR CI/CD CHANGE**:

1. **Validate Trunk Integration**:
   ```powershell
   .\scripts\trunk-cicd-monitor.ps1 -Validate -TrunkCheck
   ```

2. **Verify Merge Queue Support**:
   ```powershell
   .\scripts\merge-queue-manager.ps1 -Status -ComplianceCheck
   ```

3. **Run Health Validation**:
   ```powershell
   .\scripts\trunk-cicd-monitor.ps1 -HealthCheck -FullReport
   ```

#### Consequences of Non-Compliance
- ❌ **Rejected PRs**: Any changes bypassing the exclusive CI/CD method will be rejected
- ❌ **Build Failures**: Non-compliant pipelines will fail validation checks
- ❌ **Security Risks**: Bypassing Trunk security scanning creates vulnerabilities
- ❌ **Quality Issues**: Missing merge queue validation reduces code quality
- ❌ **Support Denial**: Issues from non-compliant deployments will not be supported

#### Compliance Verification
All CI/CD changes must pass these automated checks:
- ✅ Trunk CLI integration validation
- ✅ Merge queue trigger configuration
...
## Health Service & Microsoft Docs MCP: Mandatory Troubleshooting Resources

### Health Service Usage
- **Routine Checks**: Always run the Health Service before and after major changes, deployments, or when troubleshooting issues.
- **Problem Fixing**: If a problem cannot be resolved in one execution, Health Service diagnostics become mandatory for continued troubleshooting.
- **Integration**: Use Health Service logs and reports to guide further fixes and document all findings in related issues or PRs.

### Microsoft Docs MCP Usage
- **Documentation Reference**: Use Microsoft Docs MCP to fetch official documentation, troubleshooting guides, and configuration details for .NET, Azure, CI/CD, and related technologies.
- **Mandatory for Multi-step Fixes**: If a problem persists after initial attempts, consult Microsoft Docs MCP for authoritative solutions and step-by-step procedures.
- **Routine Inclusion**: Incorporate MCP documentation in all troubleshooting, especially for complex or recurring issues.

### Workflow for Problem Resolution
1. **Attempt Fix**: Try to resolve the issue in one execution.
2. **If Unresolved**: Immediately run Health Service diagnostics and consult Microsoft Docs MCP.
3. **Document Findings**: Record all Health Service outputs and MCP documentation links in the issue or PR.
4. **Iterative Fixes**: Continue troubleshooting using insights from Health Service and MCP until resolved.
5. **Update Documentation**: Add lessons learned and new procedures to this guide for future reference.
...
- **Health Service**: Diagnostics and health monitoring
- **Microsoft Docs MCP**: Official documentation and troubleshooting
...
- Include reproduction steps
- Provide environment details
- Attach Health Service logs and MCP documentation links
- Tag appropriate team members

---

*This document serves as a comprehensive guide for Wiley Widget development. Regular updates will reflect new best practices, mandatory troubleshooting resources, and lessons learned.*
