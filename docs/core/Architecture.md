...
Wiley Widget is a .NET 9 WPF desktop application that layers Prism 9 on top of the MVVM pattern. The application shell lives in `src/WileyWidget` and composes feature modules from sibling projects under `src/`.

## Key Points

- **Frameworks**: Prism 9, DryIoc container, WPF with Syncfusion UI components
