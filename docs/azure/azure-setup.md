...
Wiley Widget uses Azure Key Vault for enterprise-grade secret management. This section covers the complete Key Vault setup and integration.

### Key Vault Overview

- **Purpose**: Secure storage for API keys and sensitive credentials
- **Security**: RBAC authorization with audit logging
- **Integration**: Automated secret loading for development

### Automated Key Vault Setup

The setup script includes Key Vault creation:

```powershell
# Run complete setup including Key Vault
.\scripts\setup-azure.ps1 -AzureSubscriptionId "your-subscription-id" -CreateKeyVault
```
...
az role assignment create `
    ...
    --scope "/subscriptions/your-subscription/resourceGroups/wileywidget-rg/providers/Microsoft.KeyVault/vaults/wiley-widget-secrets"
```
...
az keyvault list --resource-group "wileywidget-rg"
...
1. **Verify RBAC permissions**:
   ```powershell
   az role assignment list --assignee $(az ad signed-in-user show --query id -o tsv) --scope "/subscriptions/sub/resourceGroups/wileywidget-rg/providers/Microsoft.KeyVault/vaults/wiley-widget-secrets"
   ```

2. **Check Key Vault policies** (if using access policies):
   ```powershell
   az keyvault show --name "wiley-widget-secrets" --query "properties.accessPolicies"
   ```

### Related Documentation

- 📖 **[KEYVAULT_README.md](../../KEYVAULT_README.md)**: Detailed Key Vault guide
- 🔧 **[MCP Configuration](../../MCP_CONFIGURATION_README.md)**: MCP server setup
