...
WPF projects use temporary project files (_.wpftmp.csproj) for XAML compilation, which can cause IntelliSense to use stale type information. The SDK-style project automatically includes all `_.cs`files in`src/Services/`, so no explicit `<Compile Include>` is needed.
