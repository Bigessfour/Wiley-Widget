...
class ResourceScanner:
    def __init__(self, root_path: str):
        ...
        self.definition_locations: Dict[str, List[Tuple[str, int, str]]] = defaultdict(list)  # key -> [(file, line, type)]
        self.missing_resources: Set[str] = set()
        self.duplicate_resources: Dict[str, List[Tuple[str, int, str]]] = {}  # key -> [(file, line, type)]
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                for resource_type, pattern in RESOURCE_PATTERNS.items():
                    ...
                    for match in matches:
                        ...
                        self.definition_locations[key].append((str(file_path), line_num, resource_type))

                        # Check for duplicates
                        if key in self.defined_resources:
                            if key not in self.duplicate_resources:
                                # First duplicate found - add original location
                                self.duplicate_resources[key] = list(self.definition_locations[key])
                        else:
    ...
    def _analyze(self):
        ...
        if missing_sync:
            print(f"⚠️  Missing Syncfusion brushes: {', '.join(missing_sync)} - May cause startup warnings!")
    ...
    def _report(self, output_json: str = None):
        ...
        report = {
            ...
            'critical_missing_syncfusion': sorted(list(SYNCFUSION_BRUSHES - self.defined_brushes)),
            'locations': {k: v for k, v in self.brush_locations.items() if k in self.missing_brushes},
            'recommendations': [
...
def main():
    parser = argparse.ArgumentParser(description="Scan Wiley Widget for WPF brush resources.")
