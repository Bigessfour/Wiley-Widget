...
class LegacyScanner:
    ...
    def __init__(self, root_dir: Path = Path(".")):
        ...
        self.file_extensions = {".cs", ".xaml", ".csproj", ".config", ".xml", ".targets", ".props"}
    ...
    def scan(self, max_workers: int = 4) -> List[ScanHit]:
        ...
        filtered_files = [f for f in all_files if not self.is_ignorable(f) and f.is_file()]
        ...
        if "file_summary" in report:
            ...
            for file_path, hits in sorted_files:
                ...
                for hit in hits[:3]:
                    ...
                    if hit['suggestion'] and hit == hits[0]:  # Show suggestion once per file
                        print(f"           💡 {hit['suggestion']}")
                
                if len(hits) > 3:
                    print(f"  ... and {len(hits)-3} more hits. Run with --verbose for full list.")
...
Examples:
  ...
  python legacy_scanner.py --patterns custom.json  # Use custom patterns
    ...
    parser.add_argument("--output", type=str, default="legacy_scan_report.json", 
    ...
    # Run scan
    hits = scanner.scan(max_workers=args.max_workers)
