...
@unittest.skipUnless(HAS_PYTHONNET, "pythonnet required for CLR integration tests")
class TestQuickBooksStructure(unittest.TestCase):

    def _await(self, task):
        return task.GetAwaiter().GetResult()

    def _make_fake_cloudflared_cmd(self, tmpdir: Path) -> str:
    ...
    def _build_quickbooks_service(self):
        # Load dependencies
        ...
        from System import (  # type: ignore[attr-defined]
            ...
            Activator,  # type: ignore[attr-defined]
            Array,  # type: ignore[attr-defined]
            Object,  # type: ignore[attr-defined]
            CancellationToken,  # type: ignore[attr-defined]
            CancellationTokenSource,  # type: ignore[attr-defined]
            )
        ...
        from WileyWidget.Services import (
            ISecretVaultService,  # type: ignore[attr-defined]
        )
    ...
    def test_oauth_flow_with_cloudflare_tunnel_and_refresh(self):
        ...
        service, CancellationToken, CancellationTokenSource = self._build_quickbooks_service()

        # Prepare a fake cloudflared binary
        with tempfile.TemporaryDirectory(prefix="ww_fake_cf_") as tmp:
            fake_exe = self._make_fake_cloudflared_cmd(Path(tmp))
        ...
        self._await(result_task)

    @unittest.skip("Interactive OAuth listener test is opt-in; set RUN_QB_OAUTH_TESTS=1 to enable.")
    def test_interactive_oauth_listener_handles_error_callback(self):
        ...
        with tempfile.TemporaryDirectory(prefix="ww_fake_cf_") as tmp:
            ...
            self.assertIsNotNone(ensure_cf)
            cts = CancellationTokenSource()
            cts.CancelAfter(5000)
            ready_task = ensure_cf.Invoke(service, Array[Object]([cts.Token]))
            ready = self._await(ready_task)
            ...
            _ = self._await(resp_task)  # ignore status details

            # AuthorizeAsync should return False due to error
            result = self._await(auth_task)
...
def test_expired_token_triggers_refresh_without_missingmethod(
    ...
    svc_type = service.GetType()
    settings_field = svc_type.GetField(
