...
class ResourceScanner:
    def __init__(self, root_path: str):
        ...
        self.resource_types: Dict[str, str] = (
            {}
        )  # key -> inferred type (e.g., 'Brush', 'Style')
    ...
    def _analyze(self):
        ...
        if missing_sync:
            print(
                f"⚠️  Missing Syncfusion resources: {', '.join(sorted(missing_sync)[:10])}{'...' if len(missing_sync) > 10 else ''}"
            )
    ...
    def _report(self, output_json: Optional[str] = None, syncfusion_only: bool = False):
        ...
        report = {
            ...
            "resource_types": dict(
                list(self.resource_types.items())[:50]
            ),  # Sample for top refs
            "locations": {
                k: v
        ...
        if duplicates_dict:
            ...
            for key, paths in sorted(list(duplicates_dict.items())[:10]):
                print(
                    f"  {key}: {', '.join(paths[:2])}{'...' if len(paths) > 2 else ''}"
                )
        ...
        print(
            "\n"
            + (
                "🎉 All resources accounted for - Startup ready!"
                if all_good
                else "⚠️  Action required: Review missing/duplicates."
            )
        )
