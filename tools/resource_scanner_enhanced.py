...
CONVERTER_USAGE_XAML = re.compile(
    r"Converter=\{[^}]*(?:Static|Dynamic)Resource\s+([^\s\},]+)\}", re.IGNORECASE
)
CONVERTER_USAGE_STATIC = re.compile(
    r"Converter=\{x:Static\s+[\w:]+\.([^\s\},]+)\}", re.IGNORECASE
)

# NEW: C# converter patterns
CS_CONVERTER_IMPL = re.compile(
    r"public\s+class\s+(\w+)\s*:\s*IValueConverter", re.IGNORECASE
)
CS_CONVERTER_REGISTRATION = re.compile(
    r'Resources\.Add\(["\'](\w+)["\']\s*,\s*new\s+(\w+)\(', re.IGNORECASE
)
...
RESOURCE_PATTERNS = {
    ...
    "AnimationTimeline": re.compile(
        r'<(\w+Animation(?:Timeline)?)\s+x:Key="([^"]+)"', re.IGNORECASE
    ),
}
...
SYNCFUSION_RESOURCES = {
    ...
    "SfBooleanToVisibilityConverter",
...
class EnhancedResourceScanner:
    ...
    def __init__(self, root_path: str):
        ...
        self.converter_usages: Dict[str, List[Tuple[str, int]]] = defaultdict(
            list
        )  # NEW
                    ...
                    for line_num, line in enumerate(lines, 1):
                        ...
                        for key in matches:
                            if len(key) > 2:
                                ...
                                self.converter_usages[key].append(
            ...
            if self._is_valid_scan_path(file_path):
                try:
                    ...
                    for key, class_name in reg_matches:
                        ...
                        self.resource_types[key] = (
                            f"Converter (C# registered as {class_name})"
                        )
        ...
        for file_path in self.root.rglob(f"*{RES_EXT}"):
            if self._is_valid_scan_path(file_path):
                if (
                    ...
                    or "view" in str(file_path).lower()
                ):  # Catch panel/view local resources
    ...
    def _is_valid_scan_path(self, file_path: Path) -> bool:
        ...
        return (
            file_path.is_file()
            and "bin" not in path_str
            and "obj" not in path_str
    ...
    def _analyze(self, focus: str):
        ...
        if focus == "converter":
            self.missing_resources = {
                ...
                if "converter" in self.resource_types.get(r, "").lower()
                or "converter" in r.lower()
            }
        ...
        if missing_sync:
            ...
            print(
                f"⚠️  Missing Syncfusion resources: {', '.join(sync_list)}{'...' if len(missing_sync) > 10 else ''}"
            )
    ...
    def _analyze_converters(self):
        ...
        converter_defs = {
            k for k, v in self.resource_types.items() if "converter" in v.lower()
        }
        converter_refs = {k for k in self.converter_usages.keys()}
        ...
        print(
            f"  Missing Converters: {len(converter_refs - converter_defs - self.cs_converter_classes)}"
        )

        if missing_conv := (
            converter_refs - converter_defs - self.cs_converter_classes
        ):
    ...
    def _report(self, output_json: Optional[str], syncfusion_only: bool, focus: str):
        ...
        if syncfusion_only:
            ...
            filtered_missing = {
                r for r in self.missing_resources if "converter" in r.lower()
            }
            filtered_defined = {
                r for r in self.defined_resources if "converter" in r.lower()
            }
        else:
        ...
        duplicates_dict = {
            k: v for k, v in self.duplicate_resources.items() if len(v) > 1
        }
        ...
        report = {
            ...
            "critical_missing_syncfusion": sorted(
                list(SYNCFUSION_RESOURCES - self.defined_resources)
            ),
            ...
            "locations": {
                k: v
                for k, v in self.resource_locations.items()
                if k in filtered_missing
            },
            "recommendations": [
        ...
        if focus == "converter":
            ...
            print(
                f"  C# Implementations: {report['converter_summary']['cs_implementations']}"
            )
        ...
        # Final verdict
        all_good = (
            not filtered_missing
            and not (SYNCFUSION_RESOURCES - self.defined_resources)
            and len(duplicates_dict) == 0
        )

        verdict = (
            "🎉 All resources validated - Test 70 ready!"
            if all_good
            else "⚠️  Action required: Review report"
        )
...
def main():
    ...
    parser.add_argument(
        "--focus",
        choices=["all", "converter", "brush", "style"],
        default="all",
    ...
    scanner.scan(
        output_json=args.output, syncfusion_only=args.syncfusion_only, focus=args.focus
    )
